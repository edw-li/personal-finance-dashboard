...
async function sidebarGroup(options) {
  for (const density of ['comfortable', 'compact']) await runCase('sidebar', { ...options, density }, async (page, id) => {
    ...
    check(id, 'Footer shows the full identity beside one row of visible 28px actions', observed?.identity?.visible && observed.identity.text.includes('@') && !observed.identity.truncated && observed.identity.textOverflow !== 'ellipsis' && observed.footerRows === 1 && observed.buttons.length === 2 && observed.buttons.every(b => b.visible && Math.abs(b.width - 28) <= 1 && Math.abs(b.height - 28) <= 1) && measure.spread(observed.buttons.map(b => b.top)) <= 1 && Math.abs((observed.identity.top + observed.identity.bottom) / 2 - (observed.buttons[0].top + observed.buttons[0].bottom) / 2) <= 1, { identity: observed?.identity, buttons: observed?.buttons })
