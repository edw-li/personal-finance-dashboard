...
Budget history intentionally has no GET endpoint. Its representative also performs read-only SQL against the fixed private `finance_polish_v_write` target: it refuses an occupied fixture month, compares the exact restored row including id, and verifies the whole budget table hash after cleanup. All final logs are checked and serialized after background requests settle and contexts close.
