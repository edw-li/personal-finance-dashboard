...
async def test_preferences_ride_the_export(auth_client, seeded_user):
    # Read before the export: the snapshot opens its REPEATABLE READ transaction with a rollback
    # that expires every instance loaded on the shared session (a lazy refresh here has no
    # greenlet to await on).
