...
async def test_refresh_endpoint_runs_and_reports(auth_client, db, monkeypatch):
    ...
    provider = FakeProvider({"NVDA": [bar(yesterday, "220"), bar(today, "225.5")]})
...
async def test_sparklines_held_only_weekly_downsampled(auth_client, db):
    ...
    assert resp.status_code == 200, resp.text
    body = resp.json()
    assert set(body) == {"NVDA"}  # ZM has bars but no transactions — never sparklined
    ...
    for day in days:  # ascending, so the last write per week wins
        iso = day.isocalendar()
