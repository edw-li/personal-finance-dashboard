...
describe('CalendarPage — form, arrivals, land-on-save', () => {
  ...
  it('Delete offers exact batch Undo and confirms the restoration', async () => {
    ...
    await screen.findByRole('grid')
    fireEvent.click(chipIn(DAY_15, 'Car insurance'))
    vi.mocked(fetchCalendar).mockResolvedValue(payload(fixtureEvents().filter((event) => event.id !== 41)))
    ...
    await waitFor(() => expect(document.activeElement).toBe(chipIn(DAY_15, 'Car insurance')))
    expect(screen.queryByRole('dialog')).toBeNull()
  })
