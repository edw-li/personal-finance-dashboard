...
// Depends on: charts/theme.ts (dark constants), charts/formatters.ts (the two label formatters);
// re-exports charts/offScale.ts, monthLabels.ts, partial.ts and fan.ts.
import { compactMoney, percentLabel } from './formatters'
import { INK, MUTED, SURFACE } from './theme'

// Focused modules split out of this file (the 2026-09-23 code-quality review). Every symbol is
// re-exported here, so `from './grammar'` keeps working everywhere it is imported.
export { compactMoney, percentLabel } from './formatters'
export { OFF_SCALE_LABEL_SHARE, offScaleGap, offScaleMarkPoint, offScaleMarks } from './offScale'
export type { OffScaleMark, OffScalePoint } from './offScale'
export {
  MONTH_LABEL_COMPACT_PX,
  MONTH_LABEL_FULL_PX,
  MONTH_LABEL_SHORT_PX,
  PARTIAL_MARK,
  dateAxis,
  fitMonthAxes,
  monthAxis,
...
export type { MonthLabelMode } from './monthLabels'
export { DEFICIT_DECAL, ESTIMATE_DECAL, isPartialMonth, partialItemStyle, partialNote } from './partial'
export { apportionCents, fanCents } from './fan'
