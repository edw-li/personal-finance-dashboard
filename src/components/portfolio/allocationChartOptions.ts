  ...
  return {
    ...
    series: [{
      ...
      data: data.slices.filter((s) => Number(s.market_value) > 0).map((s, i) => ({
        name: displayLabel(s.key, s.label), value: Number(s.market_value), allocationKey: s.key,
...
export function exposureCsv(data: AllocationData): ExportTable {
  return {
    ...
    rows: [...data.slices.map((s) => [
      displayLabel(s.key, s.label), s.market_value, (Number(s.weight_pct) * 100).toFixed(4),
      s.is_unknown ? UNCLASSIFIED_LABEL : 'Classified', data.scope_key, data.as_of ?? '', data.latest_quote_at ?? '',
