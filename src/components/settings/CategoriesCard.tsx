...
export default function CategoriesCard() {
  ...
  const savedName = categories.find((row) => row.id === editingId)?.name ?? ''
  const saveState = useSaveState({ dirty: form.name !== savedName })
  ...
  const submit = () => {
    ...
    void saveState.run(() => track(async () => {
      const saved = await (editingId !== null ? updateCategory(editingId, body) : createCategory(body))
