...
it('requires a confirmation for the exact selected historical set and clears it when the selection changes', async () => {
  ...
  expect(screen.getByRole('button', { name: 'Close selected months (2)' }).getAttribute('aria-disabled')).toBe('true')
  fireEvent.click(screen.getByLabelText(/I checked balances/))
...
it('keeps focus on Close, marks only that action busy, and reports success beside it for 2.5 seconds', async () => {
  let finish!: (value: { months: MonthReview[] }) => void
  mocks.close.mockReturnValue(new Promise(resolve => { finish = resolve }))
  await open()
  fireEvent.click(screen.getAllByRole('checkbox')[0])
  fireEvent.click(screen.getByLabelText(/I checked balances/))
  const button = screen.getByRole<HTMLButtonElement>('button', { name: 'Close selected months (1)' })
  button.focus()
  fireEvent.click(button)
  expect(button.disabled).toBe(false)
  expect(button.getAttribute('aria-busy')).toBe('true')
  expect(document.activeElement).toBe(button)
  expect(screen.getByRole('button', { name: 'Refresh history' }).getAttribute('aria-busy')).toBeNull()
  vi.useFakeTimers()
  await act(async () => { finish({ months: [{ ...review, state: 'closed' }] }) })
  expect(document.activeElement).toBe(button)
  expect(within(button.parentElement!).getByRole('status').textContent).toContain('Saved')
  act(() => vi.advanceTimersByTime(2500))
  expect(within(button.parentElement!).queryByRole('status')).toBeNull()
})

it('clears a failed close beside its action when the selected months change', async () => {
  mocks.close.mockRejectedValue(new ApiError('A selected month changed.', 409))
  await open()
  fireEvent.click(screen.getAllByRole('checkbox')[0])
  fireEvent.click(screen.getByLabelText(/I checked balances/))
  const button = screen.getByRole('button', { name: 'Close selected months (1)' })
  fireEvent.click(button)
  await screen.findByRole('alert')
  expect(within(button.parentElement!).getByRole('alert').textContent).toContain('A selected month changed.')
  fireEvent.click(screen.getAllByRole('checkbox')[0])
