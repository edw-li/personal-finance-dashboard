...
| --- | --- | --- |
| G0 shell | `guide/g0-shell` · `.worktrees/guide-g0` | `src/components/navItems.ts`, `routeChunks.ts`, `App.tsx`, `App.test.tsx`, `Layout.test.tsx`, `Layout.tsx` (comment only), `backend/app/services/prefs_registry.py`, new `src/pages/GuidePage.tsx` + `GuidePage.css` + `GuidePage.test.tsx`, new `src/guide/types.ts`, `src/guide/content.ts` (assembler), `src/guide/content/{start,routines,pages-tracking,pages-income,pages-planning,reference}.ts` (empty arrays + one exemplar card), `src/guide/GuideCard.tsx`, `src/guide/GuideTaskList.tsx`, `src/guide/renderSteps.tsx`, `src/guide/anchors.ts`, `src/guide/guideContent.test.ts` (the fences), `src/guide/renderSteps.test.tsx`, `src/guide/GuideCard.test.tsx`, `src/guide/palette.ts` (the entry builder — the palette wiring itself is G5) |
| G1 content: Start here + Routines | `guide/g1-start` | `src/guide/content/start.ts`, `src/guide/content/routines.ts` |
| G2 content: Tracking pages | `guide/g2-tracking` | `src/guide/content/pages-tracking.ts` (Overview, Net worth, Portfolio, Spending, Credit cards) |
| G3 content: Income + Taxes pages | `guide/g3-income` | `src/guide/content/pages-income.ts` (Paycheck, Comp, ESPP, Taxes) |
| G4 content: Planning pages + Reference | `guide/g4-planning` | `src/guide/content/pages-planning.ts` (Projection, Calendar, Settings ×2), `src/guide/content/reference.ts` |
...
**Every task id has exactly one canonical home.** A second card that needs the same task uses a **pointer task**: its own id ending in `-pointer`, one step naming the place ("Accounts are managed in **Settings → Household → Accounts**."), and `to` set to the canonical task's guide anchor (`/guide?section=pages#accounts-add`). Pointer tasks count toward a card's visible tasks. This is how Net worth points at account management, Spending at the wizard, Calendar at the reminder day's Settings card, and Reference → Asking the assistant at the key.
...
3. Bring up the dev stack (Docker Postgres 5433, uvicorn 8000, vite 5173 — the memory recipe) and run `tools/probes/guide-v/smoke.mjs` (G5 writes it): for every `to` in `GUIDE`, navigate and assert the URL landed, the `?section` tab is selected where present, and a `#id` target received focus or is in view; screenshot each chapter in dark and light at 1440×900 and 1920×1080; tab through the strip and one card's links with the keyboard; verify the Overview *Start here* card and the wizard note against an empty scratch database if one is cheap to create, otherwise against their unit tests only (state which).
