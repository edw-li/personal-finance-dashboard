...
- `.page { container-type: inline-size }` (Projection already does this on its root). `.kpi-row > :last-child { grid-column-end: -1 }`. `.kpi-row-5 { grid-template-columns: repeat(5, minmax(0, 1fr)) }` inside `@container (min-width: 1000px)`, else auto-fit with `minmax(200px, 1fr)`. `@container (max-width: 980px) { .kpi-row:not(.kpi-row-5) { grid-template-columns: repeat(2, minmax(0, 1fr)) } }`. `.kpi-row-dense` = `minmax(200px, 1fr)` (replaces Portfolio's `.tiles-row`). `.projection-outcomes` is `position: static` below the 1000px container width.
