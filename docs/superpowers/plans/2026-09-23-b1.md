...
| M2 follow-up | Both ZIP downloads stream in 64 KiB blocks — a Windows dev-box mitigation: the scratch round trip found one ~500 KB write stalling at ~457 KB when the client sent `Connection: close` (urllib 16/30, curl 12/30; the live export fails the same way on main), while the re-review found prod's Linux stack delivering it whole 300/300. After: 40/40 and 30/30 here, which reduces the risk on Windows without strictly ruling it out. | 80bb903, 5a3b6c5 |
| Re-review Important | `StoredFileResponse` closes the file however the response ends. Under uvicorn's ASGI 2.3 Starlette cancels the stream on a disconnect and never closes the generator; live here, an abandoned 40 MB download held the file 20 s+ (3/3) before, and released it within 0.1 s (5/5) after. | 9caa6c8 |
| Re-review Minor | O_NONBLOCK on the stored-file open: a FIFO named like a point is refused at once instead of hanging a worker thread (verified on Linux with the function lifted verbatim; the pytest case is POSIX-only). | 57f7da1 |
| Re-review nit | Only a restore-point source asks for rotation protection. | d41813d |
