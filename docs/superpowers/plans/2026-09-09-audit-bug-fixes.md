...
- [ ] `clock.product_today()` owns the Pacific calendar day; `scheduler.py` re-exports it.
- [ ] Every "user's calendar day" call site becomes `clock.product_today()` (module attribute, one
      patch point). Deliberate UTC timestamps stay and are listed in the report.
- [ ] Tests: `test_clock.py` (17:00+ PT instant → Pacific date; Dec 31 late PT → the year); existing
...
- [ ] Task A1 (server): `put_month` skips inserting a zero for a category with no existing row
...
- [ ] Task A2 (client): the spending body includes stored-row categories plus non-zero entries only
...
- [ ] Task A3 (health): zero-filled check also catches months with net pay and all-zero rows; same
      repair. Test with a seeded month.
- [ ] Task A4 (item 18): `selectMonth` keeps the step unless the target has no balances. Test.
- [ ] Task A5 (item 20): live/review savings rate uses `(net pay − living − tax) ÷ net pay` from
...
- [ ] Task B1 (item 2): unit per key on the wire; form kinds (count / percent ↔ fraction); server
      ranges; relabels. Tests: count renders "20", percent renders "98%" and saves `0.98`; server 422
      outside range.
- [ ] Task B2 (4a): excess deduction reduces stacked gains. Golden: agi < deduction with LTCG.
- [ ] Task B3 (4b): `state_agi` subtracts `interest_us_treasuries`. Golden.
- [ ] Task B4 (4h): §199A below the line; out of the itemized suggestion; relabel. Golden.
- [ ] Task B5 (4f): federal Base = true AGI; effective rate divides by it. Goldens move; the
      By-jurisdiction hint says "AGI including gains and qualified dividends".
- [ ] Task B6 (4g): `computable` refuses current/future single years with missing tables;
      grandfathers earlier single years. Tests for both.
- [ ] Task B7 (4e): prior-year suggestions for the three deduction keys; amber Totals warning while
      both deduction keys are absent. Tests: suggestion present/absent; warning renders.
- [ ] Task B8 (4i): anniversary-based long-term test with Feb 29 handling. Tests.
...
- [ ] Task C1: migration + model + schemas + validators for `fed_withholding_pct` /
...
- [ ] Task C2: `withholding_calc.estimate` computes per-jurisdiction legs when both rates exist;
...
- [ ] Task C3: `withholding_estimate` fills `WithholdingOut.jurisdictions` with liabilities from the
...
- [ ] Task C4: `WithholdingPanel` renders the three tiles, harbor sentences and remedies, or the
...
- [ ] Task D1 (item 6): non-null denominator; caption "auto · from N entered months". Test with a
      null month.
- [ ] Task D2 (item 7): prior-year facts when the prior Q4 date is ahead and in the window. Test with
      today = Jan 5.
- [ ] Task D3 (item 13): `showMonth` moves `activeDay` (clamped same day / today / first). Test that
      a tabbable cell exists after each navigation.
- [ ] Task D4 (item 53): formatted list amounts. Test.
- [ ] Task D5 (item 54): chip = label span (ellipsis) + amount span (never clipped); `chipText` split.
...
- [ ] Task E1: month normalization (`YYYY-MM` → first of month) in the spending builder; the
      net-worth builder honors the month. Tests with a short month.
- [ ] Task E2: ProjectionPage publishes `whatif` entries; `_projection` decodes knob and retire
      entries into `projection()` parameters. Tests: a knob entry changes the payload.
- [ ] Task E3: humanized "Seeing:" chip (page · month · owner · year/status in words). Tests.
...
- [ ] Task F1 (item 9): owner guard on the live ping. Test.
- [ ] Task F2 (item 14): not-entered months excluded from `avg12`, drawn hollow. Tests.
- [ ] Task F3 (item 15): "today" vs "on {date}". Test.
- [ ] Task F4 (item 11, Overview half): "—" tile + note for an owner with no accounts. Test.
- [ ] Task F5 (item 12): header wording for an empty scope. Test.
- [ ] Task F6 (item 39): toast with Undo when leaving `system`. Test.
- [ ] Task F7 (item 60): one product name in five places; title tests updated.
...
- [ ] Task G1 (item 10): independent feeds on Net worth and Spending with a `FeedBanner` for the
      secondary feed; `describeError` in every page's catch. Tests: summary 500 leaves charts up.
- [ ] Task G2 (item 11): empty-scope note on Net worth; `formatCurrencyCompact` sub-dollar decimals.
      Tests.
- [ ] Task G3 (item 23): `granularity` on the summary; `period` on the wire; tiles say "vs prior
...
- [ ] Task H1 (item 5, server): `remaining_checks`, `remaining_gross`, `to_cap_rate`,
...
- [ ] Task H2 (item 5, client): presets read the server figures; Max 401(k) subtracts the scenario
      Roth; disabled chips explain. Tests.
- [ ] Task H3 (item 29): coverage-aware employer HSA sentence. Test.
- [ ] Task H4 (item 27): datalist of labels + "new account" note in both forms. Tests.
